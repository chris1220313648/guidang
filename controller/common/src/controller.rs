...
use crate::scheduler::{trigger, trigger_ability, ManagerMsg, Reflector, ResourceIndex, Scheduler};
...
// 定义一个函数spawn_kubeapi，用于启动与Kubernetes API相关的异步任务。
    pub fn spawn_kubeapi(
        ...
        let (schin_tx, schin_rx) = flume::bounded(30);
        // 创建一个Scheduler输出的通道。  发出和接受管理消息
        let (schout_tx, schout_rx) = flume::bounded(30);
         // 克隆Reflector实例。
        ...
        let reflector_clone = reflector_store.clone();
        let reflector_clone_abi = reflector_store.clone();
        // trigger输入 输出
        let (schdevin_tx, schdevin_rx) = flume::bounded(30);
        let (schdevin_tx_abi, schdevin_rx_abi) = flume::bounded(30);
        // 克隆Scheduler输入通道的发送端。
        let schin_tx_clone = schin_tx.clone();
        let schin_tx_clone_abi = schin_tx.clone();
        ...
        let register_url = "http://192.168.1.244";
