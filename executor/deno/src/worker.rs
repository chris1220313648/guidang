...
impl DenoWorker {//封装和管理运行时环境
    pub fn new<M: ModuleLoader + 'static>(
        ...
        let readableability = ops::ReadableAbilities {
            abilities: run.readableability,
        };
        let writeable = {
        ...
        let writeableability = {
            let mut abilities = HashMap::new();
            for (k, v) in run.writableability {
                abilities.insert(//每个设备用其名称作为键，值是ops::DeviceSnapshot对象，后者包含设备的名称和一个空的提交HashMa
                    k,
                    ops::AbilitySnapshot {
                        name: v.name,
                        http_url:v.http_url,
            ...
            ops::WritableAbilities { abilities }
        };
        ...
        op_state.put(readableability);
        op_state.put(writeableability);
        op_state.put(envvar);
        op_state.put(client);
