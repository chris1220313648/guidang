...
use proto::server_message::{
    run_script::{Manifest as ProtoManifest, ReadDevice, WriteDevice,ReadAbility,WriteAbility},
    RunScript,
};
...
pub trait RunScriptLookup {//这个RunScriptLookup特征（trait）定义了一组方法，用于在执行脚本的上下文中查找脚本、设备，以及映射设备到脚本和查询脚本的可读写设备
    ...
    fn lookup_readable_ability(&mut self, script: &Script) -> Result<HashMap<String, ReadAbility>>;
    //lookup_readable返回一个映射，键是可读设备的标识符，值是对应的ReadDevice实例；
    fn lookup_writable_ability(&mut self, script: &Script) -> Result<HashMap<String, WriteAbility>>;
}
...
impl<T: RunScriptLookup + Send + 'static> Scheduler<T> {//这个实现块内部，你可以定义 Scheduler<T> 的方法，这些方法将能利用 T 提供的 RunScriptLookup 功能，
    ...
    pub fn lookup(&mut self, index: ResourceIndex<Script>) -> Result<ManagerMsg> {
        ...
        let readableability = self.lookup_impl.lookup_readable_ability(&script)?;
        let writableability = self.lookup_impl.lookup_writable_ability(&script)?;
...
pub type SelectorMapAbiScrs = DashMap<ResourceIndex<Ability>, DashSet<ResourceIndex<Script>>>;
...
pub struct Reflector {
    ...
    pub selector_map_abi_scrs:SelectorMapAbiScrs,
...
impl Reflector {//结构体函数
    ...
    pub fn remove_ability(&self, abi: &Ability) {
        let idx = abi.into();
        if self.ability_store.remove(&idx).is_none() {
            tracing::warn!(aility =?abi, "Reflector want to remove nonexsit Ability")
        }
    }
    ...
    pub fn add_script(&self, script: &Script) {//添加脚本，添加关系
        ...
        let abilities = self.get_selected_by_name_for_ability(script);
        debug!(script =? idx, abilities =? abilities);
        for abi in abilities {//这段代码遍历abilities列表，为每个设备更新与之关联的脚本集合
            let set = self.selector_map_abi_scrs.get(&abi);
            match set {
                None => {
                    let new_set = DashSet::new();
                    new_set.insert(script.into());
                    self.selector_map_abi_scrs.insert(abi, new_set);
                }
                Some(set) => {
                    set.insert(script.into());
                }
    ...
    fn get_selected_by_name(&self, script: &Script) -> Vec<ResourceIndex<Device>> {
        ...
        if let Some(map) = &script.spec.read_selector.match_names {
            for name in map.values() {
                if !name.is_empty() {
                    idx.name = name.clone(); // 将每个设备名称分别赋值给 idx.name
                    result.push(idx.clone()); // 克隆并加入 result
                }
            }
        }
        result
    }
    fn get_selected_by_name_for_ability(&self, script: &Script) -> Vec<ResourceIndex<Ability>> {
        let mut result = Vec::new();
        let mut idx = ResourceIndex {
            name: String::new(),
            namespace: "default".to_string(),
        ...
        if let Some(map) = &script.spec.read_selector.match_abilities {
            for name in map.values() {
                if !name.is_empty() {
                    idx.name = name.clone(); // 将每个设备名称分别赋值给 idx.name
                    result.push(idx.clone()); // 克隆并加入 result
...
impl RunScriptLookup for Arc<Reflector> {
    ...
    fn lookup_readable_ability(&mut self, script: &Script) -> Result<HashMap<String, ReadAbility>> {
        ...
        if let Some(map) = &script.spec.read_selector.match_abilities {//
            for (k, v) in map.iter() {
                ...
                let abi = if let Some(abi) = self.ability_store.get(&idx) {
                    abi
                } else {//使用idx在device_store中查询对应的设备。如果设备不存在，则跳过当前迭代。
                    continue;
                };
                ...
                for (attribute_name, item) in &abi.attributes {
                    trace!(attribute_name = ?attribute_name); // 打印属性名称用于追踪调试
                
                    // 遍历设备的状态孪生值 `twin`，假设 `s.twins` 是某个类似状态孪生的结构
                    for (property_name,value) in &item.properties {
                        
                            status.insert(property_name.to_owned(), value.to_string());
                ...
                result.insert(//对于每个处理过的设备，使用设备的标识符(k)和一个新的ReadDevice实例（包含设备名称和状态映射）更新结果哈希图result。
                    ...
                    ReadAbility {
                        name: v.to_owned(),
                        http_url:abi.http_url.to_owned(),
                        status,
    ...
    fn lookup_writable_ability(&mut self, script: &Script) -> Result<HashMap<String, WriteAbility>> {
        let mut result = HashMap::new();
        if let Some(map) = &script.spec.write_selector.match_abilities {
            for (k, v) in map.iter() {
                let idx = ResourceIndex {//设备索引  k是脚本里用的名字，v是脚本资源名字
                    namespace: "default".to_string(),
                    name: v.to_string(),
                ...
                result.insert(k.to_owned(), WriteAbility { name: v.to_owned(),http_url:abi.http_url.to_owned() });
            }//对于match_names中的每一对(k, v)，使用键值对创建WriteDevice实例，
        }
        Ok(result)
    }
...
#[tracing::instrument(skip_all)]
pub async fn trigger_ability(//段trigger异步函数的设计是在一个事件驱动的架构中，根据设备的变化自动触发与之相关联的脚本执行
    store: Arc<Reflector>,
    ability: Receiver<ResourceIndex<Ability>>,
    script: Sender<ResourceIndex<Script>>,
) -> Result<()> {
    loop {
        let idx = ability.recv_async().await?;
        info!(ability =? idx, "map trigger got new ability");
        if let Some(scripts) = store.selector_map_abi_scrs.get(&idx) {
            for s in scripts.iter() {//遍历脚本迭代器
                info!(script =? *s, "map trigger new script");
                script.send_async(s.clone()).await?;//异步发送脚本索引到另一个通道以触发脚本执行
            }
